...
function getGoodsData() {
	...
	var dataBox = '';
	var url = apiDomain + '/diyapi/goods/overview?goodsId=' + goodsId;
	var successFunc = function(data) {
		...
		function confirmAttribute(selectedStyleList) {
			...
			$('.attribute b').text($('.ap-selectedInfo').text().slice(2));
	...
	$('.attribute .back').off('click').click(function() {
		layer.open({
			...
			btn: ['确定', '取消'],
			yes: function(index) {
				//如果带有"_"的channel则表示为友宝那边的，返回就是首页了
				if(/_/.test(channel)){
					window.location.href = 'index.html?channel=' + channel;
				}else{
