function chooseAttribute(box, data, confirmAttribute, middleImg) {
	...
	*/
	...
	function disposeChecked(key, ps) {
		$('.attributePopup .ap-main .attrItemBox .attrItem[data-key="'+ key +'"]').addClass("act")
		var choosable = [];

		$.each(choosableBox, function(i) {

			if(new RegExp(key).test(choosableBox[i])) {
				choosable.push(choosableBox[i]);
			}
		});
...
//					}
		...
		//缩小可选属性集合，准备给下一次选择属性后从里面筛选
		choosableBox = choosable;

		//找到对应的前景图，模型图等
		selectedStyleList = styleList[attributeData[choosable[0]].index];
	...
	function attributePopup() {
		...
		$('.attributePopup .ap-main .attrItemBox .attrItem').off().click(function() {
			...
			//清除掉选中行 下面的 act和disabled  防止有多个已经选中导致后面出错 （就是像这个特殊的情况很难描述，只能笼统说出错，很想做个gif图）
			var ps2 = $(this).parents('.attrBox').index();
			$.each(attributeKeys, function(i) {
			if(i > ps2) {
				$('.attributePopup .ap-main .attrBox').eq(i).hide()
				$('.attributePopup .ap-main .attrBox').eq(ps2+1).show()
...
//			$('.attributePopup .ap-main .attrBox .attrItem').removeClass('disabled');
			...
			//每次点击属性的时候初始化索引大于0也就是除了第一排的属性隐藏掉
			$('.attributePopup .ap-main .attrBox:gt(0)').hide()
			//将已选的属性初始化
