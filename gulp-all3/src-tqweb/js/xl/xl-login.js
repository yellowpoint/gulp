...
function loginLayer(box,succBack){
	...
	if($(".xl-page").length == 0) {
		var dataBox = '';
		dataBox = '<div class="xl-page no-toolbar no-tabbar" data-page="binding" id="binding"><div class="page-content bind"><div class="xl-close"><img src="../../img/login/diy-closePreviewWhite.png"/></div><img src="../../img/login/bindBg.png" class="bindBg" /><h1 class="bindTitle">手机登录/注册</h1><div class="content-block"><div class="content-block-inner content-block-first"><div class="list-block"><ul><li class="phoneInput"><div class="phonePic"><img src="../../img/login/bindPhone.png"/></div><div  class="phoneN"><input type="number" pattern="[0-9]*"  placeholder="+86  请输入手机号码" class="userTelNum" minlength="11" maxlength="11" ></div></li><li class="phoneInput"><div class="codePic"><img src="../../img/login/bindMessg.png"/></div><div  class="codeN"><input type="number" placeholder="短信验证码" class="userCode" pattern="[0-9]*" maxlength="6"></div><div class="sendCode">发送验证码</div></li></ul><div class="bindNow"><button>立即登录</button></div><p class="bindText">登录/注册后即可购买您精心设计好的定制产品</p></div></div></div></div></div>'
		$(box).append(dataBox)
	} else {
		$(".xl-page").show();
	}
...
//		inputFouc();   //检测键盘是否弹起
	...
	$('.bind').on('click','.xl-act',function(){
		...
		$('.bind').on('click','.bindAct',function(){
