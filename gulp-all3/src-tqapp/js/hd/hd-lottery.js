...
function loadPrizeList() {
	var url = apiDomain + '/diyapi/lotto/sudoku/load?token=' + token,
		successFunc = function(data) {

			if(data.code == 200) {
				prizeList = data.data.list;
...
$('body').on('click', '#lottery .lottery-btn', function() {
	...
	if(click) {
		...
		var url = apiDomain + '/diyapi/lotto/draw?token=' + token + '&turnplateId=' + turnplateId,
			successFunc = function(data) {
...
$('body').on('click', '.lottery_my', function() {
	window.location.href = 'lottery_my.html?turnplateId=' + turnplateId
})
...
//设置分享内容
wxShare('乐乐定制跨年礼品节抽奖领取220元现金红包券', 'https://api.51app.cn/channel/lottery.html', 'https://api.51app.cn/channel/img/lottrty/share.png')